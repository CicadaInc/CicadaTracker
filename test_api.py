...
print(requests.post(url+'task', json={'title': 'It works!'}, params={'token': u.token}).json())
r = requests.post(url+'task', params={'token': u.token}).json()
print(requests.get(url+'task', params={'token': u.token}).json())
...
print(requests.get(url+'task', params={'token': u.token}))
print()
requests.delete(url+'task/'+str(236327238236237236723732))
requests.delete(url+'task/'+str(r['id']))
requests.delete(url+'task/'+str(r['id']), params={'token': u.token})